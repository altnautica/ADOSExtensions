...
import { useAtlasReadinessStore } from "../../src/stores/atlas-readiness-store";
...
describe("atlas-readiness-store", () => {
  ...
  it("a snapshot stored without an expiry never expires", () => {
    useAtlasReadinessStore.getState().setReadiness("dev3", readiness({ capturing: true }), null);
    expect(useAtlasReadinessStore.getState().isCapturing("dev3", Number.MAX_SAFE_INTEGER)).toBe(true);
  });
