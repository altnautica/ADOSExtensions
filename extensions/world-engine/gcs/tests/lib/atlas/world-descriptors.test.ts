...
import {
  ...
  type SplatArtifact,
} from "../../../src/lib/atlas/world-descriptors";
import {
...
describe("splat descriptor", () => {
  it("decodes the producer frame with every stated fact", () => {
    ...
    expect(a.url).toBe("http://198.51.100.5:8092/artifacts/job-1/splat.ply");
    expect(a.handle).toBeNull();
    expect(a.manifestUrl).toBe(
      "http://198.51.100.5:8092/artifacts/job-1/manifest.json",
    );
...
describe("point-cloud descriptor", () => {
  it("decodes the producer frame's count and bounds", () => {
    ...
    expect(a.url).toBe("http://198.51.100.5:8092/artifacts/job-1/cloud.ply");
