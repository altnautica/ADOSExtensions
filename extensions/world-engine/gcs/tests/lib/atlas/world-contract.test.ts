 ...
 * checked against the agent protocol crate's own values, and the envelope
 * decoder is exercised on frames captured from that producer.
 */
...
import {
  ...
  PLUGIN_ATLAS_TOPICS,
} from "../../../src/lib/atlas/world-contract";
import { asMsgpackMap, decodeMsgpack } from "../../../src/lib/atlas/msgpack";
import {
  encodeTestEvent,
  GOLDEN_EVENT_HEX,
...
describe("envelope version", () => {
  it("matches the version the golden producer frames carry", () => {
...
describe("decodeAtlasEvent", () => {
  ...
  it("refuses a frame whose payload is not a byte sequence", () => {
    ...
    const result = decodeAtlasEvent(
      hexBytes(
        "83a17601a5746f706963b9706c7567696e2e776f726c642d656e67696e652e73706c6174a77061796c6f6164a26869",
      ),
    );
    expect(result.ok).toBe(false);
    if (!result.ok) {
...
describe("shared-data capability gate", () => {
  it("maps the artifact topics to the world read capability and the pose lane to telemetry.read", () => {
    expect(ATLAS_WORLD_READ_CAP).toBe("plugin.world-engine.world.read");
  ...
  it("matches exactly, so a topic minted under a gated prefix inherits nothing", () => {
    ...
    expect(atlasTopicSubscribeCapability("plugin.world-engine.")).toBeNull();
