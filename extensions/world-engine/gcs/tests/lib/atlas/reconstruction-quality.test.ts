...
import {
  ...
  stepsForQuality,
  qualityForSteps,
} from "../../../src/lib/atlas/reconstruction-quality";
...
describe("reconstruction-quality presets", () => {
  ...
  it("bundles a gaussian-count cap + SH degree that grow with the level", () => {
    // Finer = bigger budget; the cap is the primary speed lever, so every level
    // carries a real (non-zero) bound, strictly increasing.
