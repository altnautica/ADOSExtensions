/**
 * @module atlas-store
 * @description Per-drone Atlas world-model capture telemetry: the
 * during-flight Live World state (capture session, ingest stats, the paired
 * reconstructor, and the active transport bearer), keyed by the drone's
 * device id. These are the drone's own capture facts; reconstruction metrics
 * (gaussian count, training rate) live on the compute node's World Model
 * surface, not here.
 *
 * Fed from the `atlas` slice of the extension's agent state (see
 * `use-world-engine-state`). A drone's slice stays empty (every field null)
 * until its capture service reports, so the Live World view renders an
...
interface AtlasStoreState {
  /** Live slice per drone device id. */
  live: Record<string, AtlasLiveState>;
  /** Replace a drone's live slice (the caller passes a fully-merged slice). */
  setLive: (deviceId: string, live: AtlasLiveState) => void;
}

export const useAtlasStore = create<AtlasStoreState>((set) => ({
  live: {},
  setLive: (deviceId, live) => set((s) => ({ live: { ...s.live, [deviceId]: live } })),
}));
