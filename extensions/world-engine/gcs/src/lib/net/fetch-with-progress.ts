 ...
 * @description Read a binary artifact into an `ArrayBuffer` while reporting
 ...
 * still reports received bytes and leaves `percent` null (indeterminate).
 */

import type { ArtifactSource } from "./artifact-source";
...
/**
 * Read `source` into an `ArrayBuffer`, invoking `onProgress` as bytes arrive.
...
export async function fetchArrayBufferWithProgress(
  ...
  try {
    armStall();
    const res = await Promise.race([source.fetch(controller.signal), stall.promise]);
