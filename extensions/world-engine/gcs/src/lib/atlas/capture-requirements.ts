 ...
 * and reachable (per the locked contract). The camera + service requirements
 * read the readiness as-is.
 *
...
import type { AtlasReadiness } from "../agent/atlas-control-client";
...
export interface AtlasGateInputs {
  ...
  /** Whether a workstation / compute node is paired. */
 ...
 * present, capture service enabled + running, and a compute node paired.
 * Compute reachability is surfaced as a warning but
...
export function computeCaptureGate(inputs: AtlasGateInputs): AtlasCaptureGate {
  const { readiness, computePaired, computeReachable } = inputs;
  ...
  // Reachability is a warning, not a blocker.
  const canStart = camerasMet && serviceMet && computePaired;
