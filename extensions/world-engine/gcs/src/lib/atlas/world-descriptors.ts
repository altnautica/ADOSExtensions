...
export function occupancyVoxelCount(
  ...
  const [nx, ny, nz] = artifact.dims ?? [];
  return nx !== undefined && ny !== undefined && nz !== undefined ? nx * ny * nz : null;
}
...
export function boundsExtent(
  ...
  const [x0 = 0, y0 = 0, z0 = 0, x1 = 0, y1 = 0, z1 = 0] = bounds;
  const extent = [x1 - x0, y1 - y0, z1 - z0];
