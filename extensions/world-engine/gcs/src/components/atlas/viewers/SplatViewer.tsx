 ...
 *
 * Four things this viewer must do that the library does not do for us:
 *  1. **Serve the bytes.** The library fetches a URL itself and can neither
 *     send the host's authentication nor read an object URL under the host's
 *     CSP, so it is handed a sentinel URL answered by the artifact source
 *     (see `sentinel-fetch`).
 *  2. **Pass an explicit format.** That URL's name cannot be trusted by the
 *     loader's `endsWith('.ply')` sniffing, so we derive the real format from
 *     the artifact's file name (see `splat-format`).
 *  3. **Correct the world frame.** Reconstructions are in the COLMAP Y-down
 ...
 *  4. **Frame the camera.** The library does not auto-frame; a reconstruction lives
 ...
 *
 * WebGL: `dispose()` releases the WebGL context, sort worker, and GPU buffers.
 * It is async, so the effect
...
import { serveAtSentinel, type SentinelRoute } from "../../../lib/net/sentinel-fetch";
...
export function SplatViewer({ source }: { source: ArtifactSource }) {
  ...
  useEffect(() => {
    ...
    let route: SentinelRoute | null = null;
    ...
    const run = lifecycle.current.then(async () => {
      ...
      try {
        ...
        const ext = splatArtifactExt(source.name);
        const format =
          ext === "splat"
        ...
        viewer = v;
        // The library passes its own abort signal, which `dispose()` fires. A
        // non-2xx answer makes the library throw on its own internal promise
        // without rejecting `addSplatScene`, so the failure is surfaced here.
        route = serveAtSentinel(source.name, async (init) => {
          const res = await source.fetch(init?.signal ?? new AbortController().signal);
          if (!res.ok && !disposed) setFailed(true);
          return res;
        });
        await v.addSplatScene(route.url, {
          format,
    ...
    return () => {
      ...
      // so call it now rather than after the load finishes; a newer source or an
