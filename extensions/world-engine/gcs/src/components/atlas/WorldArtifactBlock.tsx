  ...
  return (
    <div className="we:flex we:items-center we:justify-between we:text-[11px]">
      ...
      <span
        className={
          value === null
            ? "we:font-mono we:text-text-tertiary"
            : "we:font-mono we:text-text-secondary we:tabular-nums"
        }
      >
...
export function TextRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="we:flex we:items-center we:justify-between we:text-[11px]">
      <span className="we:text-text-tertiary">{label}</span>
      <span className="we:font-mono we:text-text-secondary">{value}</span>
    </div>
  );
}
...
export function WorldArtifactBlock({
  ...
  return (
    <div
      className="we:border we:border-border-default we:rounded-md we:p-2 we:space-y-1"
      data-testid={`world-artifact-${kind}`}
    >
      <div className="we:text-xs we:font-medium we:text-text-secondary">
        {t(WORLD_ARTIFACT_KEY[kind])}
      </div>
      {artifact === null ? (
        <div className="we:text-[10px] we:text-text-tertiary">
          {t("worldNotProduced")}
        </div>
      ) : (
        <>
          ...
          {artifact.unstated.length > 0 && (
            <div className="we:text-[10px] we:text-status-warning">
